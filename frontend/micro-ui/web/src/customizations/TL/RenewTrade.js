...
import TLCheckPage from "./TLCheckPage"
...
const TLRenewTrade = (  props ) => {
  ...
  let config = [];
  let application = {};
  const [params, setParams, clearParams] = window.Digit.Hooks.useSessionStorage("TL_RENEW_TRADE", {});
  let filter1 = {};
  if (licenseNo) filter1.licenseNumbers = licenseNo;
  if (tenantId) filter1.tenantId = tenantId;
  const { isLoading, isError, error, data } = window.Digit.Hooks.tl.useTradeLicenseSearch({ filters: filter1 }, { filters: filter1 });
  ...
  return (
   <Switch>
      ...
      <Route path={`${getPath(match.path, match.params)}/check`}>
        <TLCheckPage onSubmit={createProperty} value={params} />
      </Route>
