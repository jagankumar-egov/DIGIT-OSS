  ...
  useEffect(() => {
    ...
    if (payRestrictiondetails?.length) {
      if(IsDisconnectionFlow){
       ...
       setPaymentRules(payRestrictiondetails.filter((e) => e.code == businessService)[0]);
    }
    else
      setPaymentRules({
    ...
    <React.Fragment>
      ...
      {checkFSM ? (
        <StatusTable>
          ...
          />
          <Row label={t("ADV_BALANCE_AMOUNT")} textStyle={{ textAlign: "right", maxWidth: "100px" }} text={"₹ " + Number(getTotal()).toFixed(2)} />
        </StatusTable>
      ) : (
